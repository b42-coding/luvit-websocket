...
exports.disassemblePacket = function(buffer)
    ...
    if length > #bytemap.bytes then return 3 end
...
-- fixed crash when entering via http protocol on ip:port

exports.assembleHandshakeResponse = function(handshake)
    local lines = handshake:split('\r\n')
    local title = lines[1]

    if lines[3] == "Connection: Upgrade" then
        lines[1] = nil
        local data = {}
